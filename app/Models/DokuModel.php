...
class DokuModel extends Model
{
    ...
    {
        $dokuUrl = env('DOKU_URL', null);
        $urlCallback = env('CALLBACK_URL', null);
        $clinetId = env('CLIENT_ID', null);
        $expiredTime = env('EXPIREDTIME', null);
        $secretKey = env('SECRETKEY', null);
        $payCode = "INV-" . date('YmdHis');

        $targetUrl = "/checkout/v1/payment";
        $doku_url = $dokuUrl . $targetUrl;
        $doku_callback = $urlCallback;
        ...
        $signature = $this->generateSignature($header, $targetUrl, json_encode($body), $secretKey);
        $headers = array(
            'Client-Id: ' . $clinetId,
            'Request-Id: ' . $requestId,
            'Request-Timestamp: ' . $requestTimestamp,
    ...
    function checkStatus($trxCode, $payCode)
    {
        ...
        $doku_url = $dokuUrl . $targetUrl . $payCode;
        $requestId = Uuid::uuid4()->toString();
        $requestTimestamp = Carbon::now()->setTimezone('UTC')->format('Y-m-d\TH:i:s\Z');
